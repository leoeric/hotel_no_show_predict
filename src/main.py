...
from config import (
    ROOT_DIR, DB_PATH, TABLE_NAME, TARGET_COLUMN, 
    TEST_SIZE, RANDOM_STATE, MODELS, METRICS,
    CATEGORICAL_FEATURES, NUMERICAL_FEATURES
)
from data.data_loader import load_data_from_sqlite, split_data
from features.preprocessor import (
    clean_dataset, extract_additional_features, create_preprocessing_pipeline
)
from models.model_trainer import (
    create_model_pipeline, train_model, evaluate_model, save_model
)
from utils.visualization import (
    plot_feature_importance, plot_confusion_matrix, plot_roc_curve, plot_model_comparison
)
...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default=ROOT_DIR / 'models',
    ...
    parser.add_argument('--models', type=str, nargs='+', 
                        default=['logistic_regression'],  # Default to just logistic regression to save memory
                        help='Models to train')
    parser.add_argument('--no_hyperparameter_tuning', action='store_true',
                        help='Disable hyperparameter tuning')
    parser.add_argument('--plots_dir', type=str, default=ROOT_DIR / 'plots',
...
def get_feature_names_from_pipeline(pipeline, categorical_features, numerical_features):
    ...
    try:
        ...
        return [f"feature_{i}" for i in range(len(categorical_features) * 5 + len(numerical_features))]
...
def run_pipeline(args):
    ...
    if args.sample_size:
        ...
        sample_query = f"SELECT * FROM {table_name} ORDER BY RANDOM() LIMIT {args.sample_size}"
        df = load_data_from_sqlite(db_path, table_name, query=sample_query)
        logger.info(f"Using a sample of {args.sample_size} records to reduce memory usage")
    else:
    ...
    for model_name in args.models:
        ...
        if args.no_hyperparameter_tuning:
            ...
            if args.sample_size:
                ...
                for param, values in model_config['params'].items():
                    simplified_params[param] = [values[0]] if isinstance(values, list) and values else values
        ...
        # Save model
        save_model(
            trained_model, model_name, output_dir, 
        ...
        plot_confusion_matrix(
            y_test, y_pred, 
            output_path=plots_dir / f"{model_name}_confusion_matrix.png"
        )
        
        # ROC curve
        plot_roc_curve(
            y_test, y_pred_proba,
            output_path=plots_dir / f"{model_name}_roc_curve.png"
        )
